 ...
 const assignProjectToUsers = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (newAssignees.length > 0) {
            ...
            const usersToUpdate = users.filter(user => Array.isArray(user.projectsId) && !user.projectsId.includes(projectId));
        ...
        // Return response
        return res.status(200).json(new ApiResponse(200, "Project assigned to users successfully"));
    } catch (error) {
...
const getAllAssignedProjects = asyncHandler(async (req, res) => {
    try {
        ...
        throw new ApiError(500, "Error fetching assigned projects: " + error.message);
    }
});
